...
class mosfet:
    ...
    def get_intrinsic_carrier_density(self, temperature):
        ...
        ni = decimal.Decimal(ncnv_sqrt) * decimal.Decimal(-1 * self.bandgap(temperature) / (2 * self.BOLTZMANN * temperature)).exp()
        return ni

    def __cosh(self, x: decimal.Decimal) -> decimal.Decimal:
        return (x.exp() + (decimal.Decimal(-1) * x).exp()) / decimal.Decimal(2)

    def get_vth_on(self, temperature, Vdd):
        ni = self.get_intrinsic_carrier_density(temperature)
        surface_potential = decimal.Decimal(0.4) + decimal.Decimal(self.BOLTZMANN * temperature) * ((decimal.Decimal(self.variables.ndep) / ni).ln())
        xdep = (decimal.Decimal(2 * self.ESI) * surface_potential / decimal.Decimal(self.CHARGE * self.variables.ndep * 1000000)).sqrt()
        lt0 = (decimal.Decimal(3 * self.variables.toxm) * xdep).sqrt()
        V_shift_dibl = decimal.Decimal(-0.5 * (self.variables.eta0) * Vdd) / (self.__cosh(decimal.Decimal(self.variables.dsub * self.variables.leff) / lt0) - decimal.Decimal(1))
        V_shift_sch = decimal.Decimal(-0.5 * self.variables.dvt0 * 2) / (self.__cosh(decimal.Decimal(self.variables.dvt1 * self.variables.leff) / lt0) - decimal.Decimal(1)) * \
            (decimal.Decimal(self.BOLTZMANN * temperature) * ((decimal.Decimal(self.variables.ndep * self.variables.nsd) / (ni * ni)).ln()) - decimal.Decimal(surface_potential))
        ...
        Vth_on = decimal.Decimal(self.Vth0) + V_shift_dibl + V_shift_sch
        return float(Vth_on)
    ...
    def get_vth_off(self, temperature, Vdd):
        ...
        surface_potential = decimal.Decimal(0.4) + decimal.Decimal(self.BOLTZMANN * temperature) * ((decimal.Decimal(self.variables.ndep) / ni).ln())
        xdep = (decimal.Decimal(2 * self.ESI) * surface_potential / decimal.Decimal(self.CHARGE * self.variables.ndep * 1000000)).sqrt()
        lt0 = (decimal.Decimal(3 * self.variables.toxp) * xdep).sqrt()
        V_shift_dibl = decimal.Decimal(-0.5 * (self.variables.eta0) * Vdd) / (self.__cosh(decimal.Decimal(self.variables.dsub * self.variables.leff) / lt0) - decimal.Decimal(1))
        V_shift_sch = decimal.Decimal(-0.5 * self.variables.dvt0 * 2) / (self.__cosh(decimal.Decimal(self.variables.dvt1 * self.variables.leff) / lt0) - decimal.Decimal(1)) * \
            (decimal.Decimal(self.BOLTZMANN * temperature) * ((decimal.Decimal(self.variables.ndep * self.variables.nsd) / (ni * ni)).ln()) - decimal.Decimal(surface_potential))
        #surface_potential = 0.4 + self.BOLTZMANN * temperature * log(self.variables.ndep / ni)
        ...
        #V_shift_dibl = -0.5 / (cosh(self.variables.dsub * self.variables.leff / lt0) - 1) * (self.variables.eta0) * Vdd
        #V_shift_sch = -0.5 * self.variables.dvt0 / (cosh(self.variables.dvt1 * self.variables.leff / lt0) - 1) * 2 * (self.BOLTZMANN * temperature * log(self.variables.ndep * self.variables.nsd / (ni**2)) - surface_potential)
        Vth_off = decimal.Decimal(self.Vth0) + V_shift_dibl + V_shift_sch
        return float(Vth_off)
    ...
    def calc_currents(self):
        ...
        surface_potential = float(decimal.Decimal(0.4) + decimal.Decimal(self.BOLTZMANN * self.temperature) * ((decimal.Decimal(self.variables.ndep) / ni).ln()))
        ...
        self.Isub = decimal.Decimal(I0) * (decimal.Decimal(1) - decimal.Decimal(-1 * self.Vdd / self.BOLTZMANN / self.temperature).exp()) * \
            decimal.Decimal((-1 * self.Vth_off - (self.variables.voff + self.variables.voffl / self.variables.leff)) / ((1 + (mfactor - 1) * self.variables.nfactor) * self.BOLTZMANN * self.temperature)).exp()
        ...
        vauxigc = decimal.Decimal(self.variables.nigc * (self.BOLTZMANN * self.temperature)) * \
            decimal.Decimal(decimal.Decimal(1) + decimal.Decimal(-1 * (vgse - self.Vth_on) / self.variables.nigc / (self.BOLTZMANN * self.temperature)).exp()).ln()
        ...
        if (self.mos_type == 1):
            igc0 = float(decimal.Decimal(self.AGCN * toxratio * vgse * self.variables.leff * 1e-6) * vauxigc *
                         decimal.Decimal(-1 * self.BGCN * self.variables.toxm * (self.variables.aigc - self.variables.bigc * voxdepinv) * (1 + self.variables.cigc * voxdepinv)).exp())
            ...
            igs = self.AGSDN * toxratio_edge * vgs * vgs_primed * exp(-1 * self.BGSDN * self.variables.toxm * self.variables.poxedge * (self.variables.aigsd -
                                                                      self.variables.bigsd * vgs_primed) * (1 + self.variables.cigsd * vgs_primed)) * self.variables.lint * 1e-6
            igd = self.AGSDN * toxratio_edge * vgd * vgd_primed * exp(-1 * self.BGSDN * self.variables.toxm * self.variables.poxedge * (self.variables.aigsd -
                                                                      self.variables.bigsd * vgd_primed) * (1 + self.variables.cigsd * vgd_primed)) * self.variables.lint * 1e-6
        if (self.mos_type == 2):
            igc0 = float(decimal.Decimal(self.AGCP * toxratio * vgse * self.variables.leff * 1e-6) * vauxigc *
                         decimal.Decimal(-1 * self.BGCP * self.variables.toxm * (self.variables.aigc - self.variables.bigc * voxdepinv) * (1 + self.variables.cigc * voxdepinv)).exp())
            ...
            igs = self.AGSDP * toxratio_edge * vgs * vgs_primed * exp(-1 * self.BGSDP * self.variables.toxm * self.variables.poxedge * (self.variables.aigsd -
                                                                      self.variables.bigsd * vgs_primed) * (1 + self.variables.cigsd * vgs_primed)) * self.variables.lint * 1e-6
            igd = self.AGSDP * toxratio_edge * vgd * vgd_primed * exp(-1 * self.BGSDP * self.variables.toxm * self.variables.poxedge * (self.variables.aigsd -
                                                                      self.variables.bigsd * vgd_primed) * (1 + self.variables.cigsd * vgd_primed)) * self.variables.lint * 1e-6
